...
struct KefirCLI: AsyncParsableCommand {
    ...
    static func runFirstTimeSetup() async {
        ...
        guard let host = readLine()?.trimmingCharacters(in: .whitespacesAndNewlines), !host.isEmpty
        else {
...
struct SpeakerOptions: ParsableArguments {
    @Option(
        name: [.short, .long], help: "Speaker name or IP address (uses default if not specified)")
    var speaker: String?
    ...
    func resolveSpeaker() async throws -> (host: String, name: String) {
        ...
        if let speakerArg = speaker {
            ...
            throw ValidationError(
                "No speaker specified and no default configured. Use 'kefir speaker add' to configure a speaker."
            )
        }
    }
}
    ...
    struct SourceSet: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            guard let kefSource = KEFSource(rawValue: source.lowercased()) else {
                throw ValidationError(
                    "Invalid source. Available sources: \(KEFSource.allCases.map { $0.rawValue }.joined(separator: ", "))"
                )
            }
